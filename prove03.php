<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
   <title>Shopping Cart</title>
</head>

<div class="top">

    <div style="background-color: #102b35;">

        <div align="right" style="background-color: #415962;">
            <input type="text" name="search" placeholder="Search.." style="background-color: #e7eaeb;">
        </div>
   
        <div class="square" align="left">
            <a target="_self" href="https://lit-wave-14499.herokuapp.com/index.html">
                <img src="http://pngimages.net/sites/default/files/man-pushing-a-shopping-cart-png-image-74122.png" alt="Cart." width="250" height="150">
            </a>    
        </div>
            <h1 class="title"; align="center";>Hahn Kandies</h1>
            <ul>
                <li><a class="others" href="https://lit-wave-14499.herokuapp.com/index.html" target="_self">Index</a></li>
            </ul>
        </div>
        
    <h1 align="center">Store</h1>
    <h2 align="center">Unfortunately (for some people) we only have M&Ms right now.</h2>

    <div id="main">
        <form action="cart.php" method="post">
            <table style="width:100%">
              <tr>
                <th>Item</th>
                <th>Image</th>
                <th>Price</th>
                <th>Add to cart</th>
              </tr>
              <tr>
                <td>Plain M&Ms</td>
                <td><a target="_blank"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/Plain-M%26Ms-Pile.jpg/280px-Plain-M%26Ms-Pile.jpg" alt="Plain M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="Plain" type="text" value="1" size="2"></input></td>
              </tr>
              <tr>
                <td>Peanut M&Ms</td>
                <td><a target="_blank"><img src="http://img.wennermedia.com/920-width/mj-618_348_five-rules-for-a-healthier-halloween-stay-full-with-peanut-m-ms.jpg" alt="Peanut M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="product[]" type="text" value="1" size="2"></input></td>
              </tr>
              <tr>
                <td>Peanut Butter M&Ms</td>
                <td><a target="_blank"><img src="http://cdn2.blisstree.com/wp-content/uploads/2009/07/strawberriedpbmms3.jpg" alt="Peanut Butter M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="product[]" type="text" value="1" size="2"></input></td>
              </tr>
              <tr>
                <td>Holiday Mint M&Ms</td>
                <td><a target="_blank"><img src="http://photos1.blogger.com/blogger/1431/1452/1600/IMG_0849.jpg" alt="Holiday Mint M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="product[]" type="text" value="1" size="2"></input></td>
              </tr>
              <tr>
                <td>Dark Chocolate M&Ms</td>
                <td><a target="_blank"><img src="https://i2.wp.com/farm5.static.flickr.com/4135/4870443325_8ac0f1c0b2.jpg" alt="Dark Chocolate M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="product[]" type="text" value="1" size="2"></input></td>
              </tr>
              <tr>
                <td>Peppermint M&Ms</td>
                <td><a target="_blank"><img src="http://farm8.staticflickr.com/7336/11166171813_4b76c7c77b_o.jpg" alt="Peppermint M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="product[]" type="text" value="1" size="2"</input></td>
              </tr>
              <tr>
                <td>MEGA M&Ms</td>
                <td><a target="_blank"><img src="http://www.bernardandagnes.com/food/images/mm.jpg" alt="MEGA M&Ms." width="100" height="50"></a></td>
                <td>10</td>
                <td><input name="MEGA" type="text" value="1" size="2"</input></td>
              </tr>
            </table>
            <label>See Cart</label>
            <input type="submit" value="Submit">
        </form>
    </div>
</div>

<div class="side"></div>

<!-- Here is the use of inline style -->
<div style="position: relative; vertical-align:middle; height: 10px">    

   <div style="position: absolute; bottom: 15px; margin: auto; left:40%; font-family: Times New Roman, Times, Serif">
      <span style="font-weight: bold; font-size: small;"> &copy;  2017 "Hahn Kandy Supplies Co.". All Rights Reserved.</span> 
   </div>
</div>

</body>
</html>